 ...
 * @LastEditTime: 2023-03-15 21:49:53
 * @Description: CSS 面试题汇总（50题）
 * @Version: 1.0.0
...
CSS（Cascading Style Sheets）是一种用于控制 Web 页面样式和布局的语言，它可以与 HTML 结合使用，用于设计和美化网站。CSS 通过对 HTML 标记的属性和值的定义，使得 Web 页面具有更好的排版、样式和视觉效果。

CSS 的特性包括：

1. **分离样式和内容**：CSS 将网页的样式和内容分离开来，使得网页结构更加清晰，同时也方便修改和维护。
1. **层叠样式**：CSS 中的样式可以层叠在一起，这意味着多个样式可以同时应用到同一个元素上，最终形成一个样式叠加的效果。
1. **继承样式**：CSS 中的样式可以继承父元素的样式，这使得网页的样式设计更加方便和高效。
1. **多种选择器**：CSS 提供了多种选择器，可以根据元素的标签名、类名、ID、属性等选择元素，从而实现更加精确的样式控制。
1. **可重用样式**：CSS 中的样式可以定义为类和 ID，这些样式可以在多个元素中重复使用，避免了重复编写样式的麻烦。

总之，CSS 是一种用于控制 Web 页面样式和布局的语言，具有分离样式和内容、层叠样式、继承样式、多种选择器和可重用样式等特性。它使得网页设计更加灵活、高效和易于维护。
...
1. 一种方法是使用 transform: scaleY(0.5) 把线条的高度缩小为原来的一半。这种方法在某些情况下会产生锯齿或模糊的效果。
2. 另一种方法是使用 CSS3 的 box-shadow 属性来模拟线条，例如：
...
CSS 深度选择器是一种选择器，它可以选择嵌套在其他选择器中的元素。在 Vue.js 框架中，也有一个类似的概念，称为 “vue-deep”，它可以在单个组件中使用深度选择器，以便在样式中选择嵌套的子组件。

CSS 深度选择器使用 “/” 符号来表示嵌套关系。例如，如果我们想为一个类为 “.outer” 的元素中的一个类为 “.inner” 的元素设置样式，可以使用以下选择器：

```
.outer .inner {
  /* styles for inner element */
}
```

这将选择类为 “.outer” 的元素内的类为 “.inner” 的元素，并为其设置样式。

在 Vue.js 中，可以使用 “/deep/” 或 “::v-deep” 伪选择器来表示深度选择器。例如，如果我们想选择嵌套在父组件中的子组件的元素，可以使用以下选择器：
...
在上面的例子中，“.parent” 是父组件，而 “child” 是子组件。在样式中，我们使用 “/deep/” 或 “::v-deep” 选择器来选择嵌套在父组件中的子组件的元素，并为其设置样式。

#### CSS 深度选择器原理？

需要注意的是，深度选择器可能会影响样式的性能和可维护性。因此，应尽量避免过度使用深度选择器，并在必要时使用其他解决方案来管理样式。

CSS 深度选择器（也称为 CSS 作用选择器或 CSS 嵌套选择器）是一种 CSS 选择器，它允许您选择嵌套在其他选择器内的元素。

其原理是基于 CSS 的级联和继承机制。当我们使用 CSS 深度选择器时，它会通过层叠样式表中的级联关系来匹配嵌套元素的样式。在选择器中使用了深度选择器后，就可以通过父元素来控制子元素的样式，从而实现样式的嵌套和继承。

具体来说，当使用一个 CSS 深度选择器时，浏览器会首先匹配外层选择器，然后在匹配的元素中继续查找满足内层选择器的元素，直到找到最终匹配的元素为止。在这个过程中，只有当内层选择器完全匹配时，才会选择这个元素。

例如，当我们使用.parent .child 选择器时，浏览器会首先查找所有具有.parent 类的元素，然后在这些元素中继续查找具有.child 类的元素，最终选择所有同时具有这两个类的元素。

需要注意的是，CSS 深度选择器会增加选择器的复杂度和计算时间，过多的使用深度选择器可能会导致性能问题。因此，在实际开发中，应该尽量避免过多的使用深度选择器，尤其是在嵌套层级较深的情况下。
