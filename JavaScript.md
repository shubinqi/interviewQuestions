 ...
 * @LastEditTime: 2023-03-19 22:14:02
 * @Description: JavaScript 面试题汇总（141题）
 * @Version: 1.1.0
...
1. **解释性语言**是指在执行程序时，将程序代码一行一行解释成机器代码并立即执行。解释性语言在运行时需要不断解释代码，因此通常执行速度比较慢。代表性的解释性语言有 JavaScript、Python、Ruby 等。
1. **编译型语言**则是在运行程序之前需要将程序代码编译成机器语言，并将其存储在可执行文件中。编译型语言在编译时会对代码进行优化，因此在执行时速度通常比解释型语言快。代表性的编译型语言有 C、C++、Java 等。
...
- 词法环境是一种规范类型，用于定义 ECMAScript 代码内标识符与变量值和函数值之间的关联关系，是标识符与它们的值之间的映射关系，类似于作用域链。
- 一个词法环境由环境记录（Environment Record）和一个可能为 null 的对外部词法环境的引用（Outer Lexical Environment）组成。
- 词法环境经常用于关联一个函数声明、代码块语句、try-catch 语句，当它们的代码被执行时，词法环境被创建出来。
- 变量环境是词法环境的一种特殊类型，用于存储变量声明和函数声明的绑定关系，是词法环境的一部分。
- 变量环境中的变量声明会被“提升”（hoisting）到作用域顶部，但是初始化的值不会被提升。
- 在 JavaScript 中，每个函数都会创建一个新的词法环境，该环境与函数的作用域相关联。函数的词法环境包括一个环境记录器，用于存储变量和函数的绑定关系，以及一个外部词法环境的引用，该引用指向函数被创建时的作用域。
- 在 JavaScript 中，每个代码块也会创建一个新的词法环境，该环境与代码块的作用域相关联。代码块的词法环境包括一个环境记录器，用于存储变量的绑定关系，以及一个外部词法环境的引用，该引用指向代码块被创建时的作用域。
- 全局执行上下文也有一个词法环境和一个变量环境，用于存储全局变量和函数的绑定关系。
- 词法环境和变量环境的作用是为 JavaScript 中的标识符提供一个查找的路径，以便在执行代码时能够找到正确的变量或函数。
...
如果不使用 var 声明变量，而是直接给变量赋值，则该变量会被视为全局变量（如果在函数内部定义，则是函数作用域的变量），但不会像使用 var 声明的变量那样发生变量提升。

变量提升是指在执行代码之前，JavaScript 引擎会将所有使用 var 声明的变量提升到当前作用域的顶部。这意味着在声明变量之前使用变量是允许的，因为变量已经被提升到了作用域的顶部。但是，如果不使用 var 声明变量，JavaScript 引擎将不会进行变量提升，因此在声明变量之前使用变量将导致错误。

例如，下面的代码中，myVar 由于使用了 var 声明，所以会被提升到函数作用域的顶部，因此在函数内部任何地方都可以使用它：
...
但是，如果我们不使用 var 声明变量，而是直接给变量赋值，例如：

```
function myFunction() {
  console.log(myVar); // 输出错误，因为 myVar 还没有被声明
  myVar = "Hello";
  console.log(myVar); // 输出 "Hello"
}
```

在这种情况下，由于没有使用 var 声明变量，JavaScript 引擎不会进行变量提升，因此在声明变量之前使用它会导致错误。

#### 函数与 var 声明的变量同名哪个优先级高？

如果在函数作用域内使用 var 声明一个变量，并且同时在同一作用域内定义了一个函数，且函数名称与变量名称相同，那么函数的优先级更高，函数将覆盖变量。这是因为在 JavaScript 中，函数声明会被提升到当前作用域的顶部，并且会覆盖同名的变量声明。
...
在这个例子中，我们在函数 myFunction 中声明了一个变量 x，并且在同一作用域内定义了一个函数 x。由于函数声明会被提升到作用域顶部，并且会覆盖同名的变量声明，因此函数 x 会覆盖变量 x 的声明。因此，输出的结果将是 "function"。

但是，需要注意的是，如果我们使用函数表达式而不是函数声明来定义同名的函数和变量，则变量的优先级更高，变量将覆盖同名的函数。
...
在这个例子中，我们在函数 myFunction 中声明了一个变量 x，并且在同一作用域内定义了一个函数表达式 x。由于函数表达式不会被提升到作用域顶部，因此变量 x 的声明会生效，覆盖同名的函数表达式 x。因此，输出的结果将是 "number"。
...
1. **Unicode** 是一种字符集，定义了每个字符对应的唯一编号，可以看作是字符和数字的映射关系。UTF（Unicode Transformation Format）是一种编码方式，将 Unicode 中的数字编码为字节序列，以便存储和传输。
1. **UTF-8** 是一种可变长度的编码方式，使用 1-4 个字节表示每个字符，适用于存储英文、数字、符号等单字节字符，也能够表示中文、日文等多字节字符。UTF-8 最大的优点是向后兼容 ASCII 编码，它的前 128 个字符与 ASCII 编码完全相同。
1. **UTF-16** 也是一种可变长度的编码方式，使用 2 或 4 个字节表示每个字符，适用于存储英文、数字、符号等单字节字符，以及中文、日文等多字节字符。与 UTF-8 不同，UTF-16 对于字符的编码方式是固定的，不存在字节序列的问题。
1. **UTF-32** 是一种固定长度的编码方式，使用 4 个字节表示每个字符，适用于存储所有字符。UTF-32 对于每个字符都分配了固定的空间，因此可以快速访问任何一个字符，但相应的存储空间会更大。
...
1. **Ajax**：是一种基于原生 XMLHttpRequest 对象实现的技术，用于在不刷新页面的情况下与服务器进行数据交互。
1. **Axios**：是一个基于 Promise 的 HTTP 库，可以用于浏览器和 Node.js，支持拦截请求和响应，自动转换 JSON 数据，取消请求等特性。
1. **Fetch**：是浏览器原生提供的一个基于 Promise 的网络请求 API，支持 Request 和 Response 对象，可以更灵活地控制请求和响应。相比于 Ajax 和 Axios，Fetch 更加简洁，并且默认不会发送或接收任何 cookies。
...
Promise 有三种状态（Promise 对象的状态只能由 pending 转变为 fulfilled 或 rejected，一旦状态变化，就不会再改变。）：
...
匿名函数是没有名称的函数，可以直接使用函数表达式来定义。它的作用有以下几个：

1. **用于一次性的函数调用**：在某些情况下，我们只需要定义一个函数并立即执行，而这个函数在之后不会再被使用到。这时候就可以使用匿名函数，将它定义在括号内，然后通过括号运算符来立即调用它。
...
2. **用于作为函数参数**：在很多函数中，我们可以把一个函数作为另一个函数的参数传递进去。而如果这个函数只是临时需要使用一下，我们就可以使用匿名函数来避免定义过多的函数。
...
3. **用于闭包**：匿名函数可以形成闭包，使得函数内部的变量可以在函数执行完毕后继续存在。
...
在这个例子中，我们定义了一个计数器函数 counter()，它返回了一个匿名函数。这个匿名函数形成了一个闭包，可以访问 counter() 函数内部的变量 count，并且在每次执行时将 count 的值加一。由于我们将这个匿名函数赋值给了 c，所以我们可以多次调用 c() 来获取计数器的值。
...
**hasOwnProperty** 方法：hasOwnProperty 方法是用来判断某个属性是否是属于自身对象的，该方法仅用于判断自身对象，不会查找原型链。
...
**instanceof** 方法：instanceof 是从原型链上寻找是否有此实例，返回的是原型链最顶点的数据类型。
