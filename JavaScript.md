 ...
 * @LastEditTime: 2023-03-17 20:56:26
...
1. 词法环境是一种规范类型，用于定义 ECMAScript 代码内标识符与变量值和函数值之间的关联关系，是标识符与它们的值之间的映射关系，类似于作用域链。
1. 一个词法环境由环境记录（Environment Record）和一个可能为 null 的对外部词法环境的引用（Outer Lexical Environment）组成。
1. 词法环境经常用于关联一个函数声明、代码块语句、try-catch 语句，当它们的代码被执行时，词法环境被创建出来。
1. 变量环境是词法环境的一种特殊类型，用于存储变量声明和函数声明的绑定关系，是词法环境的一部分。
1. 变量环境中的变量声明会被“提升”（hoisting）到作用域顶部，但是初始化的值不会被提升。
1. 在 JavaScript 中，每个函数都会创建一个新的词法环境，该环境与函数的作用域相关联。函数的词法环境包括一个环境记录器，用于存储变量和函数的绑定关系，以及一个外部词法环境的引用，该引用指向函数被创建时的作用域。
1. 在 JavaScript 中，每个代码块也会创建一个新的词法环境，该环境与代码块的作用域相关联。代码块的词法环境包括一个环境记录器，用于存储变量的绑定关系，以及一个外部词法环境的引用，该引用指向代码块被创建时的作用域。
1. 全局执行上下文也有一个词法环境和一个变量环境，用于存储全局变量和函数的绑定关系。
1. 词法环境和变量环境的作用是为 JavaScript 中的标识符提供一个查找的路径，以便在执行代码时能够找到正确的变量或函数。
